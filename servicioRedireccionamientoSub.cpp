...
cServicioRedireccionamientoSub::cServicioRedireccionamientoSub(cDominio *dom):cServicio(dom) {
  servicioDB = "REDIRECCIONAMIENTO SUBDOMINIO";
}

bool cServicioRedireccionamientoSub::iniciar() {
  bool resultado = true;
  cServicio::iniciar();
  traerLimite();
  traerCantidad();
...
int cServicioRedireccionamientoSub::agregarFileHttpdConf(const std::string &redir,
							 const std::string &subdominio,
							 cDominio &dominio) {
  ...
  //Agregar las lineas
  out << endl
      << "#################### Redirection Subdomain: www." << redir << " ####################" << endl
      << "<VirtualHost " << SERVER_IP << ":80>" << endl
      << "ServerAdmin info@" << dominio.getDominio() << endl
      << "RedirectMatch temp ^/$ http://www." << subdominio << "." << dominio.getDominio() << endl
...
int cServicioRedireccionamientoSub::agregarFileNamed(const std::string &redir,
						     cDominio &dominio) {
  std::string archivo;
  Tiempo tiempo;
...
int cServicioRedireccionamientoSub::agregarFileNamedConf(const std::string &redir,
							 cDominio &dominio) {
  //Abrir el archivo para escritura
...
int cServicioRedireccionamientoSub::agregarRedireccionamientoSub(const std::string &redireccionamiento, const std::string &subdominio, cDominio &dominio) {
  std::string comando;
  ...
  if (agregarFileHttpdConf(redireccionamiento, subdominio, dominio) < 0) {
...
int cServicioRedireccionamientoSub::agregarRedireccionamientoSubDB(const std::string &redireccionamiento, const std::string &subdominio, cDominio &dominio) {
  try {
    Query qry = dominio.con.query();
    qry << "INSERT INTO REDIRECCIONAMIENTO_SUBDOMINIO(REDIRECCIONAMIENTO, ID_DOMINIO, "
	<< "SUBDOMINIO) VALUES('" << redireccionamiento << "', '" << dominio.getIdDominio() 
	<< "', '" << subdominio << "')";
    qry.execute();
    return(0);
  } catch(BadQuery er) {
    ...
    string error;
    error = "Error al agregar el redireccionamiento a subdominio " + redireccionamiento;
    dominio.loguear(error);
    return(-2);
  }
}

int cServicioRedireccionamientoSub::quitarFileHttpdConf(const std::string &redir,
							cDominio &dominio) {
  ...
  buscada = "#################### Redirection Subdomain: www." + redir + " #";
  hasta = "</VirtualHost>";
  int estado = 1;
...
int cServicioRedireccionamientoSub::quitarFileNamed(const std::string &redir,
						    cDominio &dominio) {
...
int cServicioRedireccionamientoSub::quitarFileNamedConf(const std::string &redir,
							cDominio &dominio) {
  //Mover el archivo para actualizarlo
...
int cServicioRedireccionamientoSub::quitarRedireccionamientoSub(const std::string &redireccionamiento, cDominio &dominio) {
  std::string comando;
...
int cServicioRedireccionamientoSub::quitarRedireccionamientoSubDB(const std::string &redireccionamiento, cDominio &dominio) {
  try {
    Query qry = dominio.con.query();
    qry << "DELETE FROM REDIRECCIONAMIENTO_SUBDOMINIO WHERE REDIRECCIONAMIENTO = '"
    ...
    string error;
    error = "Error al quitar el redireccionamiento " + redireccionamiento;
    dominio.loguear(error);
    return(-2);
  }
}

std::string cServicioRedireccionamientoSub::traerCantidad() {
  try {
    Query qry = dominio->con.query();
    qry << "SELECT COUNT(*) FROM REDIRECCIONAMIENTO_SUBDOMINIO WHERE ID_DOMINIO = '"
...
int cServicioRedireccionamientoSub::verificarExisteRedireccionamientoSubDB(const std::string &redireccionamiento, cDominio &dominio) {
  int resultado = 0;
  try {
    Query qry = dominio.con.query();
    qry << "SELECT COUNT(*) FROM REDIRECCIONAMIENTO_SUBDOMINIO WHERE REDIRECCIONAMIENTO = '"
	<< redireccionamiento << "'";
    Result res = qry.store();
    Row row;
    Result::iterator i;
    ...
    string error;
    error = "Error al verificar si existe el redireccionamiento a subdominio "
      + redireccionamiento;
    dominio.loguear(error);
    return(resultado);
  }
...
int cServicioRedireccionamientoSub::verificarExisteRedireccionamientoSubDB(const std::string &redireccionamiento, cDominio &dominio, std::string) {
  int resultado = 0;
  try {
    Query qry = dominio.con.query();
    qry << "SELECT COUNT(*) FROM REDIRECCIONAMIENTO_SUBDOMINIO WHERE REDIRECCIONAMIENTO = '"
