...
cServicioRedireccionamiento::cServicioRedireccionamiento(cDominio *dom):cServicio(dom) {
  servicioDB = "REDIRECCIONAMIENTO";
}

bool cServicioRedireccionamiento::iniciar() {
  bool resultado = true;
  cServicio::iniciar();
  traerLimite();
  traerCantidad();
...
int cServicioRedireccionamiento::agregarFileHttpdConf(const std::string &redir,
						      cDominio &dominio) {
  ...
  //Agregar las lineas
  out << endl
      << "#################### Redirection Domain: www." << redir << " ####################" << endl
      << "<VirtualHost " << SERVER_IP << ":80>" << endl
      << "ServerAdmin info@" << dominio.getDominio() << endl
      << "RedirectMatch temp ^/$ http://www." << dominio.getDominio() << endl
...
int cServicioRedireccionamiento::agregarFileNamed(const std::string &redir,
						  cDominio &dominio) {
  std::string archivo;
  Tiempo tiempo;
...
int cServicioRedireccionamiento::agregarFileNamedConf(const std::string &redir,
						      cDominio &dominio) {
  //Abrir el archivo para escritura
...
int cServicioRedireccionamiento::agregarRedireccionamiento(const std::string &redireccionamiento,
							   cDominio &dominio) {
  std::string comando;
  ...
  if (agregarFileHttpdConf(redireccionamiento, dominio) < 0) {
...
int cServicioRedireccionamiento::agregarRedireccionamientoDB(const std::string &redireccionamiento,
							     cDominio &dominio) {
  try {
    Query qry = dominio.con.query();
    qry << "INSERT INTO REDIRECCIONAMIENTO(ID_DOMINIO, REDIRECCIONAMIENTO) VALUES('"
	<< dominio.getIdDominio() << "', '" << redireccionamiento << "')";
    qry.execute();
    return(0);
  } catch(BadQuery er) {
    ...
    string error;
    error = "Error al agregar el redireccionamiento " + redireccionamiento;
    dominio.loguear(error);
    return(-2);
  }
}

int cServicioRedireccionamiento::quitarFileHttpdConf(const std::string &redir,
						     cDominio &dominio) {
  ...
  buscada = "#################### Redirection Domain: www." + redir + " #";
  hasta = "</VirtualHost>";
  int estado = 1;
...
int cServicioRedireccionamiento::quitarFileNamed(const std::string &redir,
						 cDominio &dominio) {
...
int cServicioRedireccionamiento::quitarFileNamedConf(const std::string &redir,
						     cDominio &dominio) {
  //Mover el archivo para actualizarlo
...
int cServicioRedireccionamiento::quitarRedireccionamiento(const std::string &redireccionamiento,
							    cDominio &dominio) {
  std::string comando;
...
int cServicioRedireccionamiento::quitarRedireccionamientoDB(const std::string &redireccionamiento,
							    cDominio &dominio) {
  try {
    Query qry = dominio.con.query();
    qry << "DELETE FROM REDIRECCIONAMIENTO WHERE REDIRECCIONAMIENTO = '"
...
std::string cServicioRedireccionamiento::traerCantidad() {
  try {
    Query qry = dominio->con.query();
    qry << "SELECT COUNT(*) FROM REDIRECCIONAMIENTO WHERE ID_DOMINIO = '"
...
int cServicioRedireccionamiento::verificarExisteRedireccionamientoDB(const std::string &redireccionamiento, cDominio &dominio) {
  int resultado = 0;
  try {
    Query qry = dominio.con.query();
    qry << "SELECT COUNT(*) FROM REDIRECCIONAMIENTO WHERE REDIRECCIONAMIENTO = '"
	<< redireccionamiento << "'";
    Result res = qry.store();
    Row row;
    Result::iterator i;
    ...
    string error;
    error = "Error al verificar si existe el redireccionamiento " + redireccionamiento;
    dominio.loguear(error);
    return(resultado);
  }
}

int cServicioRedireccionamiento::verificarExisteRedireccionamientoDB(const std::string &redireccionamiento, cDominio &dominio, std::string) {
  int resultado = 0;
  try {
    Query qry = dominio.con.query();
    qry << "SELECT COUNT(*) FROM REDIRECCIONAMIENTO WHERE REDIRECCIONAMIENTO = '"
